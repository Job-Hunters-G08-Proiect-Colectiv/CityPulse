...
function App() {
  ...
  const fetchReports = async () => {
    // Do not fetch until a city is selected; avoids pulling all cities
    if (!selectedCityId) {
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    const checkForUpdates = async () => {
      try {
        if (!selectedCityId) return; // keep scope to a chosen city
        ...
        if (!isCancelled) {
              ...
              if (changedMine.length) {
                ...
                setTimeout(() => {
                  setNotifications((prevNotes) =>
                    prevNotes.filter((n) => !ids.includes(n.id))
                  );
                }, 6000);
              }
            }
          ...
          const filtered = latest.filter((r) => {
            if (selectedCityId && r.cityId !== selectedCityId) return false;
            if (filterCategory !== "ALL" && r.category !== filterCategory)
              return false;
            if (filterStatus !== "ALL" && r.status !== filterStatus)
              return false;
            if (filterSeverity !== "ALL" && r.severityLevel !== filterSeverity)
              return false;
            if (searchTerm) {
  ...
  const handleCreateReport = async (
    reportData: Omit<Report, "id" | "date" | "status" | "upvotes">
  ) => {
    try {
  ...
  const handleUpvote = (reportId: number, newCount: number) => {
    setReports(
      reports.map((r) => (r.id === reportId ? { ...r, upvotes: newCount } : r))
    );
